<template>
  <v-card :loading="loading" class="mx-auto my-12" max-width="374">
    ...
    <v-card-text>
      ...
      <div class="my-4 text-subtitle-1"> {{infCard.Year}} • {{infCard.Rated}} • {{infCard.Runtime}}</div>
    ...
    <v-card-text>
      ...
      >
        ...
        <v-chip
        v-for="genero of infCard.Genre.split(',')" :key="genero"
        >
            {{genero}}
        </v-chip>
    ...
    <v-card-actions>
      <v-btn color="deep-purple lighten-2" text @click="reserve">
        Favoritar <!-- depois colocar o icon do coração -->
      </v-btn>
    </v-card-actions>
  </v-card>
</template>
